...
/** Une ligne du roster Workforce Live : profil org. visible + fiche `employees` éventuelle (sinon pas de contrat RH). */
export type PresenceComplianceRosterEntry = { profileId: string; employee: Employee | null };
...
export function computePresenceCompliance(params: {
  /** Rétrocompat : paie moteur & fiches salariés uniquement. */
  employees?: Employee[];
  /** Même roster que la grille Workforce Live (profils org. + fiche RH optionnelle). Prime sur `employees` si non vide. */
  workforceRoster?: PresenceComplianceRosterEntry[];
  sessions: PresenceSession[];
  ...
  const roster: PresenceComplianceRosterEntry[] =
    params.workforceRoster && params.workforceRoster.length > 0
      ? params.workforceRoster
      : (params.employees ?? []).map((employee) => ({ profileId: String(employee.profileId), employee }));

  return roster.map(({ profileId: profileIdRaw, employee }) => {
    ...
    // Sans ligne `employees` : pas de taux contrat RH — 0 sauf reprise éventuelle sur les sessions.
...
/** Données live par salarié pour agrégation département (Workforce Live). */
export type PresenceLiveRollupRow = {
  profileId: string;
...
export type DepartmentPresenceCardModel = {
  id: string;
  label: string;
  presentPct: number;
  absentCount: number;
...
function rollupPresenceSlice(slice: PresenceLiveRollupRow[]): DepartmentPresenceCardModel {
  const n = Math.max(1, slice.length);
  const presentNow = slice.filter((r) => ['online', 'present'].includes(r.currentStatus as PresenceStatus)).length;
...
/**
 * Cartes « heatmap » par département à partir de `user_departments` + roster présence.
 * Si aucun département ou aucune affectation : une seule carte synthétique sur toute l’équipe.
 */
export function buildDepartmentPresenceCardsFromOrg(args: {
  departments: Array<{ id: string; name: string }>;
  userDepartmentLinks: Array<{ userId: string; departmentId: string }>;
  liveRows: PresenceLiveRollupRow[];
  userIdByProfile: Record<string, string>;
  unassignedLabel: string;
  allOrgFallbackLabel: string;
}): { cards: DepartmentPresenceCardModel[]; usedDepartmentAssignments: boolean } {
  const { departments, userDepartmentLinks, liveRows, userIdByProfile, unassignedLabel, allOrgFallbackLabel } = args;

  const userToDepts = new Map<string, Set<string>>();
  for (const link of userDepartmentLinks) {
    if (!userToDepts.has(link.userId)) userToDepts.set(link.userId, new Set());
    userToDepts.get(link.userId)!.add(link.departmentId);
  }

  const deptToProfileIds = new Map<string, Set<string>>();
  const unassignedProfileIds = new Set<string>();

  for (const row of liveRows) {
    const authId = userIdByProfile[row.profileId] || row.profileId;
    const depts = userToDepts.get(String(authId));
    if (!depts || depts.size === 0) {
      unassignedProfileIds.add(row.profileId);
      continue;
    }
    for (const did of depts) {
      if (!deptToProfileIds.has(did)) deptToProfileIds.set(did, new Set());
      deptToProfileIds.get(did)!.add(row.profileId);
    }
  }

  const hasDeptDefinitions = departments.length > 0;
  const hasLinks = userDepartmentLinks.length > 0;

  if (!hasDeptDefinitions || !hasLinks) {
    const r = rollupPresenceSlice(liveRows);
    return {
      cards: [{ ...r, id: 'org-all', label: allOrgFallbackLabel }],
      usedDepartmentAssignments: false,
    };
  }

  const cards: DepartmentPresenceCardModel[] = [];
  const deptOrder = new Map(departments.map((d, i) => [d.id, i]));

  for (const d of [...departments].sort((a, b) => (deptOrder.get(a.id) ?? 0) - (deptOrder.get(b.id) ?? 0))) {
    const pids = deptToProfileIds.get(d.id);
    const slice = pids ? liveRows.filter((r) => pids.has(r.profileId)) : [];
    if (slice.length === 0) continue;
    const r = rollupPresenceSlice(slice);
    cards.push({ ...r, id: d.id, label: d.name });
  }

  if (unassignedProfileIds.size > 0) {
    const slice = liveRows.filter((r) => unassignedProfileIds.has(r.profileId));
    const r = rollupPresenceSlice(slice);
    cards.push({ ...r, id: 'unassigned', label: unassignedLabel });
  }
  ...
  return { cards, usedDepartmentAssignments: true };
}
