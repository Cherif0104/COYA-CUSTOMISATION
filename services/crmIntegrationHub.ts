...
export type CrmOutboundEvent =
  ...
  | {
      kind: 'studio.quote_created' | 'studio.invoice_created';
...
export function dispatchCrmOutboundEvent(event: CrmOutboundEvent): void {
  ...
  if (event.kind === 'studio.quote_created' || event.kind === 'studio.invoice_created') {
    void import('./crmActivityService')
      .then(({ logStudioCrmActivity }) =>
        logStudioCrmActivity({
          contactId: event.contactId,
          kind: event.kind === 'studio.quote_created' ? 'quote' : 'invoice',
          summary: event.summary,
          payload: event.payload,
        }),
      )
      .catch((err) => console.warn('[crmIntegrationHub] studio activity', err));
  }
}
