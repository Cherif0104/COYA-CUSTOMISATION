...
export async function listBailleurs(organizationId?: string | null): Promise<Bailleur[]> {
  try {
    ...
    // `select('id,...,contact')` provoque un 400 PostgREST si la colonne `contact` n’existe pas encore en base.
    // `*` renvoie toutes les colonnes présentes sans erreur de projection.
