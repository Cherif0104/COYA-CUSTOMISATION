...
function shouldUseLocal(table: string, error?: any): boolean {
  ...
  // Ne jamais basculer en local sur refus RLS / droits : sinon les DM semblent « envoyés » alors que rien n’est persisté.
  const isRlsOrAuth =
...
export async function createOrGetDirectThread(params: {
  ...
  const unique = Array.from(new Set([params.createdById, ...params.memberIds.filter(Boolean)]));
