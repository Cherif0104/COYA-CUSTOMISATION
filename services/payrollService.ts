...
/** Bulletins d’un salarié (toutes périodes), tri récents d’abord. */
export async function listPaySlipsForProfile(
  profileId: string,
  organizationId?: string | null
): Promise<PaySlip[]> {
  const slips = await listPaySlips(organizationId);
  return slips.filter((s) => String(s.profileId) === String(profileId));
}

/** Accusé RPC `rh_payroll_close_period_stub` (traçabilité / extension serveur). */
export async function rhPayrollClosePeriodStubAck(
  periodStart: string,
  periodEnd: string
): Promise<{ ok: boolean; payload?: Record<string, unknown>; error?: string }> {
  try {
    const { data, error } = await supabase.rpc('rh_payroll_close_period_stub', {
