...
const KnowledgeBase: React.FC<KnowledgeBaseProps> = ({ documents, users = [], projects = [], onAddDocument, onUpdateDocument, onDeleteDocument }) => {
    ...
    const filteredDocuments = useMemo(() => {
        let filtered = documents.filter(doc => {
            // Recherche
            const bodyText = `${doc.content || ''} ${doc.description || ''}`;
    ...
    const handleViewDocument = async (doc: Document) => {
        let docToUse = doc;
        if (!doc.content?.trim()) {
            const full = await DataAdapter.getDocumentById(doc.id);
            if (full) {
                docToUse = { ...full, isFavorite: doc.isFavorite ?? full.isFavorite };
            }
        }
        setViewingDocument(docToUse);
        // Incrémenter le compteur de vues
        if (user?.profileId && docToUse.id) {
            try {
                await supabase.rpc('increment_document_view', {
                    document_uuid: docToUse.id,
                    viewer_profile_id: user.profileId
                });
                await onUpdateDocument({
                    id: docToUse.id,
                    viewCount: (docToUse.viewCount || 0) + 1,
                });
            } catch (error) {
    ...
    const handleEdit = (doc: Document) => {
        void (async () => {
            ...
            if (!doc.content?.trim()) {
                const full = await DataAdapter.getDocumentById(doc.id);
                if (full) d = { ...full, isFavorite: doc.isFavorite ?? full.isFavorite };
            }
            ...
            setEditIsPublic(d.isPublic || false);
            setEditSharedProfileIds(d.sharedProfileIds || []);
            setEditSharedDepartmentIds(d.sharedDepartmentIds || []);
            setEditSharedProjectIds(d.sharedProjectIds || []);
            const acl = await DataService.getDocumentAcl(d.id);
            if (acl) {
                    ...
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        <div className="flex items-center justify-between">
                            <div className="flex-1">
                                <h1 className="text-4xl font-bold mb-2">{t('coya_drive_title') || 'COYA Drive'}</h1>
                                <p className="text-emerald-50 text-sm max-w-2xl">
                                    {t('coya_drive_subtitle') ||
