...
const PresenceEmployeeDetailPanel: React.FC<PresenceEmployeeDetailPanelProps> = ({
  ...
  /** Double rAF : laisse le portail / layout se stabiliser avant le montage Recharts (évite removeChild DOM). */
  const [chartDomReady, setChartDomReady] = useState(false);
  useEffect(() => {
    if (!open || chartData.length === 0) {
      setChartDomReady(false);
      return;
    }
    let r1 = 0;
    let r2 = 0;
    ...
    r1 = requestAnimationFrame(() => {
      r2 = requestAnimationFrame(() => setChartDomReady(true));
    });
    return () => {
      cancelAnimationFrame(r1);
      cancelAnimationFrame(r2);
    };
  }, [open, chartData.length, fromIso, toIso, profileId]);
  ...
  const chartKey = `presence-bar-${profileId}-${fromIso}-${toIso}`;
  ...
  return createPortal(
    <div className="fixed inset-0 z-[80] flex justify-end">
      <div
        ...
        onClick={onClose}
      />
      <aside className="relative z-[90] flex max-h-full w-full max-w-xl flex-col overflow-hidden border-l border-slate-200 bg-white shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="h-56 w-full">
            {chartData.length === 0 ? (
              ...
              <p className="text-sm text-slate-500 text-center py-8">{fr ? 'Préparation du graphique…' : 'Preparing chart…'}</p>
            ) : (
