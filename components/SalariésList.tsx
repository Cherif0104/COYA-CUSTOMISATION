...
interface SalariésListProps {
  ...
  /** Workspace runtime RH-4 (`/hr/employees/:profileId`) — requiert `profileId`. */
  onOpenWorkspace?: (employee: Employee) => void;
...
const SalariésList: React.FC<SalariésListProps> = ({
  users,
  onSelectEmployee,
  onOpenWorkspace,
  listVersion = 0,
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th className="px-4 py-3 font-semibold text-slate-700 w-40">{fr ? 'Actions' : 'Actions'}</th>
            </tr>
          </thead>
          <tbody>
            {employees.length === 0 ? (
