...
import { downloadPaySlipPdf, printPaySlipPdf } from '../services/paySlipPdfExport';
...
const PaySlipDetailDrawer: React.FC<PaySlipDetailDrawerProps> = ({
  open,
  slip,
  displayName,
  fr,
  ...
  const runPdf = async (mode: 'download' | 'print') => {
    ...
    try {
      const opts = {
        ...
        orgLogoUrl: organization?.logoUrl ?? null,
        fr,
      };
      if (mode === 'download') await downloadPaySlipPdf(opts);
      else await printPaySlipPdf(opts);
    } catch (e) {
      console.error(e);
      alert(fr ? 'Échec génération PDF.' : 'PDF generation failed.');
    } finally {
      setPdfBusy(false);
    }
  };
  ...
  return createPortal(
    <>
      {/* div plutôt que button plein écran : évite conflits focus/DOM avec React 19 + extensions */}
      <div
        role="presentation"
        ...
        <div className="space-y-4 p-4">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm font-medium text-emerald-900 hover:bg-emerald-100 disabled:opacity-50"
            >
